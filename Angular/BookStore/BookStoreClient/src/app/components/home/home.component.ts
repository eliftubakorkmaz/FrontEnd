...
import { BestsellerModel } from '../../models/bestseller.model';
...
export class HomeComponent implements OnInit {
  ...
  getBestsellers() {
    this.http.get<BestsellerModel[]>('http://localhost:5051/api/Home/Bestsellers/').subscribe({
      next: (res: any) => {
  ...
  getNewBooks() {
    this.http.get<BestsellerModel[]>('http://localhost:5051/api/Home/GetNewBooks/').subscribe({
      next: (res: any) => {
        this.newBooks = res;
      },
      error: (err: HttpErrorResponse) => {
  ...
  getLastComments() {
    this.http.get<BestsellerModel[]>('http://localhost:5051/api/Home/GetLastComments/').subscribe({
      next: (res: any) => {
